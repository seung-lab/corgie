...
@corgie_option(
    ...
    default=3.0,
    help="""
    A set of vectors can be considered "in consensus" if their consensus score is within the
    CONSENSUS_THRESHOLD of the lowest scoring set. The vector with the highest priority (the
    vector generated by the nearest section) that is considered "in consensus" will be considered
    the consensus vector for that location.
""",
)
@corgie_option(
    "--blur_sigma",
    ...
    default=15.0,
    help="""
    Determining the consensus vectors for all locations can be considered a one-hot weighting.
    This will introduce artificial non-smoothness in the final displacement field as vectors
    generated by one section transition to those generated by another section. This is adjusted
    by blurring the weightings so that the transitions are smoothed. BLUR_SIGMA controls the
    standard deviation of the Gaussian kernel used to blur. For more information see
    https://github.com/seung-lab/torchfields.
    """,
)
@corgie_option(
    "--kernel_size",
    ...
    type=int,
    default=32,
    help="""KERNEL_SIZE controls the
    width of the blurring kernel described in BLUR_SIGMA.
    """,
)
...
@corgie_option(
    ...
    default=3,
    help="""
    The number of sections of overlap that will be used to generate stitch fields.
    If STITCH_SIZE > 1, there will be multiple estimates of the stitch field, and the final stitch
    field will be voted for consensus.
    """,
)
@corgie_option(
    "--vote_dist",
    ...
    default=1,
    help="""
    The number of previously aligned sections to use as target sections during block alignment.
    If VOTE_DIST > 1, then there will be multiple estimates of the block field, and the final block
    field will be voted for consensus.
    """,
)
@corgie_option(
    ...
    default=100,
    help="""
    The maximum distance (measured in sections) for which a previous starter section may
    influence the final displacement field of a later section after stitching. Currently, the
    stitch field for a starter section is attenuated before being composed with a section's
    block field to produce the final displacement field. This attenuation is a single coefficient,
    min(max(1.0 - (CurrentSection - StarterSection) / DECAY_DIST, 0.0), 1.0).
    """,
)
@corgie_option(
    "--blur_rate",
    ...
    type=float,
    default=0.2,
    help="""The rate of increase in the width of the blurring kernel used during stitching as distance of
    a section from a previous starter section increases.  Can be roughly considered the change
    in size of the standard deviation of a Gaussian kernel per single section (we currently
    create the downsample hierarchy with a box filter). For example, if the standard deviation
    should increase by 1 px every 10 sections, then set BLUR_RATE = 0.1.
    """,
)
@corgie_option(
    "--restart_stage",
