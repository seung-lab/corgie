...
class RenderJob(scheduling.Job):
    def __init__(self, src_stack, dst_stack, mips, pad, render_masks,
                 ...
                 additional_fields=[], seethrough_offset=-1, preserve_zeros=False):
...
class RenderTask(scheduling.Task):
    def __init__(self, src_stack, dst_stack, additional_fields, render_masks,
            blackout_masks, seethrough_offset, seethrough_mask_layer, mip,
            preserve_zeros,
            pad, bcube):
        super().__init__(self)
        self.src_stack = src_stack
        ...
        for l in write_layers:
            ...
            if l.get_layer_type() == "img":
                ...
                if self.preserve_zeros and agg_field is not None:
                    src_zero_mask = src == 0
                    warped_zero_mask = residuals.res_warp_img(src_zero_mask.float(), agg_field)
                ...
                if self.seethrough_mask_layer is not None:
                    seethrough_mask = self.seethrough_mask_layer.read(bcube=self.bcube, mip=self.mip) != 0
