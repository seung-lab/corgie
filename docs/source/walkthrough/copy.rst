...
Note that we are not copying the whole example stack, but just a 10 section cutout. 
The command specifies the region of interest through ``--start_coord`` and ``--end_coord``. 
...
Lastly, we need to specify Precomputed chunking parameters. 
